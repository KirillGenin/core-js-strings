...
function formatTime(minutes, seconds) {
  const formatTimeNow = (time) => time.toString().padStart(2, '0');

  return `${formatTimeNow(minutes)}:${formatTimeNow(seconds)}`;
}
...
function isPalindrome(str) {
  const format = str.toLowerCase().replace(/[\W_]/g, '');
...
function invertCase(str) {
  return str
    ...
    .map(
      ...
      // /[a-z]/.includes(char) ? char.toUpperCase() : char.toLowerCase()
    )
    .join('');
}
